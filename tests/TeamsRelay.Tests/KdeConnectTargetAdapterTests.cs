...
public sealed class KdeConnectTargetAdapterTests
{
    ...
    {
        private readonly Dictionary<string, Queue<KdeCommandResult>> _results = new(StringComparer.Ordinal);

        public void Add(IReadOnlyList<string> arguments, int exitCode, string output)
        {
            var key = Key(arguments);
            if (!_results.TryGetValue(key, out var queue))
            {
        ...
        {
            if (_results.TryGetValue(Key(arguments), out var queue) && queue.Count > 0)
            {
