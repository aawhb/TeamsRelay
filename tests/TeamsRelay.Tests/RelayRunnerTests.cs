...
public sealed class RelayRunnerTests
{
    ...
    private static async Task<IReadOnlyList<RelayLogEntry>> ReadLogEntriesAsync(string logPath)
    {
        ...
        return lines
            ...
            .Select(line => JsonSerializer.Deserialize<RelayLogEntry>(line, JsonDefaults.Parsing) ?? throw new InvalidOperationException("Failed to parse relay log entry."))
            .ToArray();
    }
    ...
    {
        private readonly Queue<RelaySourceRecord> _records;

        public FakeSourceAdapter(IEnumerable<RelaySourceRecord> records)
        {
            _records = new Queue<RelaySourceRecord>(records);
        }
