...
public sealed class TeamsNotificationTextAnalyzerTests
{
    ...
    [InlineData("Message preview This is a test notification Message preview.")]
    [InlineData("This is a test notification | Message preview.")]
    [InlineData("Message preview. | This is a test notification")]
    public void DetectsTestNotificationEvenWhenMessagePreviewIsInline(string extractedText)
    {
        var result = TeamsNotificationTextAnalyzer.Analyze(extractedText);

        Assert.True(result.ContainsTestNotificationSegment);
        Assert.True(result.IsExactTestNotification);
    }
