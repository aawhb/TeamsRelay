...
public sealed class CliApplicationTests
{
    ...
    {
        var root = TestHelpers.CreateTemporaryDirectory();
        var stateStore = new LifecycleStateStore(RuntimePaths.Create(new AppEnvironment(root)));
    ...
    public async Task StopStopsTrackedRelayAndAuxiliaryProcesses()
    {
        ...
        var stateStore = new LifecycleStateStore(RuntimePaths.Create(environment));
        var trackedProcessId = Process.GetCurrentProcess().Id;
        await stateStore.WriteAsync(new InstanceMetadata
        {
    ...
    [Fact]
    public void RepoLocalProcessFinderMatchesRepoLocalExecutableProcess()
    {
        var root = TestHelpers.CreateTemporaryDirectory();
        ...
        var matches = RepoLocalProcessFinder.IsRepoLocalTeamsRelayProcess(process, root);

        Assert.True(matches);
    }

    [Fact]
    public void RepoLocalProcessFinderMatchesRepoLocalDotnetHostedProcess()
    {
        var root = TestHelpers.CreateTemporaryDirectory();
    ...
    [Fact]
    public void RepoLocalProcessFinderIgnoresDotnetProcessOutsideRoot()
    {
        var root = TestHelpers.CreateTemporaryDirectory();
        var process = new LocalProcessInfo(
            1,
    ...
    private static async Task InitializeConfig(string root)
    {
        ...
        var json = System.Text.Json.JsonSerializer.Serialize(RelayConfig.CreateDefault(), JsonDefaults.Parsing);
        await File.WriteAllTextAsync(configPath, json);
    }

    private sealed class FakeProcessOperations : IProcessOperations
    {
        private readonly Dictionary<int, Queue<bool>> _runningSequences = [];
        ...
        public IProcessHandle Spawn(System.Diagnostics.ProcessStartInfo startInfo)
            => throw new NotSupportedException("FakeProcessOperations does not support Spawn.");
        ...
        {
            if (_runningSequences.TryGetValue(processId, out var sequence) && sequence.Count > 0)
            {
        ...
        {
            _runningSequences[processId] = new Queue<bool>(states);
        }
    }
}
