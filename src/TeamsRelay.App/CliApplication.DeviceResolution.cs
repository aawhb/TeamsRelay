...
public sealed partial class CliApplication
{
    ...
    {
        var config = await _configFiles.LoadAsync(options.ConfigPath, cancellationToken);
        var inventory = await _targetAdapter.GetDeviceInventoryAsync(config.Config, cancellationToken);
    ...
    {
        _stdout.WriteLine("Paired devices:");
        for (var index = 0; index < inventory.Count; index++)
        {
            var device = inventory[index];
            _stdout.WriteLine($" [{index + 1}] {device.Name} ({device.Id}) [{(device.IsAvailable ? "online" : "offline")}]");
        }

        while (true)
        {
            _stdout.Write("Select device indices (comma/range, e.g. 1,3-4): ");
            var input = _stdin.ReadLine();
            if (input is null)
            {
            ...
            {
                _stdout.WriteLine("Selection required.");
                continue;
            }
            ...
            {
                _stdout.WriteLine(exception.Message);
            }
        }
    }
