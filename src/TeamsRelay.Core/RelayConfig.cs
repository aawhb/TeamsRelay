...
public sealed class RelayConfig
{
    ...
    public static RelayConfig NormalizeAndValidate(RelayConfig? config)
    {
        ...
        var memorySnapshotIntervalSeconds = config.Runtime?.MemorySnapshotIntervalSeconds ?? 0;
        if (memorySnapshotIntervalSeconds < 0)
        {
            errors.Add("runtime.memorySnapshotIntervalSeconds: must be greater than or equal to 0.");
        }

        var uiaSubscriptionMode = Normalize(config.Runtime?.UiaSubscriptionMode, "both").ToLowerInvariant();
        if (uiaSubscriptionMode is not "both" and not "window_opened_only" and not "structure_changed_only")
        {
            errors.Add("runtime.uiaSubscriptionMode: must be 'both', 'window_opened_only', or 'structure_changed_only'.");
        }
...
public sealed class RuntimeOptions
{
    ...
    public int MemorySnapshotIntervalSeconds { get; init; }

    public string UiaSubscriptionMode { get; init; } = "both";
}
