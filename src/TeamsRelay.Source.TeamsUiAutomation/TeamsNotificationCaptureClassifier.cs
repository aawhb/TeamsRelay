...
public sealed class TeamsNotificationCaptureClassifier
{
    ...
    public IReadOnlyList<RelaySourceRecord> Process(RelaySourceRecord rawRecord)
    {
        ...
        if (string.Equals(rawEventKind, RelayEventKinds.WindowOpened, StringComparison.OrdinalIgnoreCase))
        {
            HandleWindowOpened(rawRecord, ready);
        }
        else if (string.Equals(rawEventKind, RelayEventKinds.StructureChanged, StringComparison.OrdinalIgnoreCase))
        {
    ...
    private void HandleWindowOpened(RelaySourceRecord rawRecord, List<RelaySourceRecord> ready)
    {
        ...
        var candidate = CloneCandidate(
            rawRecord,
            eventKind: RelayEventKinds.WindowOpened,
            rawEventKind: RelayEventKinds.WindowOpened,
            capturePath: RelayCapturePaths.WindowOpened);

        // Park as pending so a follow-up structure_changed can enrich with body text.
        _pendingCandidates.Add(new PendingWindowOpenedCandidate(
            candidate,
    ...
    private void HandleStructureChanged(RelaySourceRecord rawRecord, List<RelaySourceRecord> ready)
    {
        ...
        {
            _pendingCandidates.Remove(bestMatch);
        ...
        if (TeamsNotificationSurfaceClassifier.IsFromNotificationToast(rawRecord) && ShouldCreateStandaloneBannerCandidate(textAnalysis))
        {
            ready.Add(CloneCandidate(
                rawRecord,
                eventKind: RelayEventKinds.StructureChanged,
                rawEventKind: RelayEventKinds.StructureChanged,
                capturePath: RelayCapturePaths.StructureChangedBanner));
            return;
        }
    }
    ...
    private static RelaySourceRecord CreateEnrichedCandidate(RelaySourceRecord windowOpenedRecord, RelaySourceRecord structureChangedRecord)
    {
        return windowOpenedRecord with
        {
            ...
            CapturePath = RelayCapturePaths.WindowOpenedEnriched,
    ...
    private static RelaySourceRecord CreateTestNotificationCandidate(RelaySourceRecord source)
    {
        return source with
        {
            EventKind = RelayEventKinds.StructureChanged,
            RawEventKind = RelayEventKinds.StructureChanged,
            CapturePath = RelayCapturePaths.StructureChangedTest,
